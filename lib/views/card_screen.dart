        ...
        body: Consumer<Cards>(
          builder: (builder, cards, child){
            ...
            return trelloCard == null
                ? const Text('No card', style: TextStyle(fontFamily: 'LexendExa'),)
                ...
                children:  [
                  Row(
                    children: [
                      ...
                      CustomIconEdit(onPressed: () => context.go('/edit-card/$cardId')),
                      CustomIconDelete(onPressed: () => showDialog(context: context, builder: (context){
                        return AlertDialog(
                  ...
                  Row(
                    children: [
                      Badge(
                        ...
                        label: Text(
                          trelloCard.dueComplete ? "completed" : "not completed",
                      ...
                      Badge(
                        ...
                        label: Text(
                          trelloCard.subscribed ? "subscribed" : "not subscribed",
                          style: const TextStyle(
                            fontFamily: 'LexendExa',
                            color: Colors.black,
                          ),
                  ...
                  Row(
                    children: [
                      if(trelloCard.idMembers == null)  const Text('no member assigned', style: TextStyle(fontFamily: 'LexendExa'),),
                      if(trelloCard.idMembers!.isEmpty) const Text('no member assigned', style: TextStyle(fontFamily: 'LexendExa'),),
                      const SizedBox(width: 20),
                      Row(
                        children: trelloCard.idMembers!.map((memberId){
                          var member = Provider.of<Members>(context, listen: false).membersById[memberId];
                          return member == null
                              ? const Text('no member', style: TextStyle(fontFamily: 'LexendExa'),)
                              : Container(
                                  ...
                                  content: Column(
                                    children: [
                                      for(var member in Provider.of<Members>(context, listen: false).members)
                                        ListTile(
                                          title: Text(member.username, style: const TextStyle(fontFamily: 'LexendExa')),
                                          onTap: (){
